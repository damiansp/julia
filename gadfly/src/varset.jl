# Generate large types where each field has an optional value to which it is 
# initialized. The format of each row in `table` is:
# "[doc str], variable name, [variable type, [default value, [depwarn str]]]"
...
macro varset(name::Symbol, table)
  ...
  for row in table
    ...
    elseif row.head == :tuple && 2 <= length(row.args) <= 5
      ...
      default = (
        length(row.args) - hasdocstr > 2 ? row.args[hasdocstr + 3] : :nothing)
      depwarnstr = (
        length(row.args) - hasdocstr > 3 ? row.args[hasdocstr + 4] : "")
    else
